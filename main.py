...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # 실제 프로덕션에서는 React 앱의 주소만 허용하세요.
...
# 프로덕션 환경에서는 Redis나 DB 사용을 권장합니다.
SESSION_DATA: Dict[str, Any] = {}

# --- 기존 로직 함수들 (Gradio에서 가져옴) ---
# (ohms_law, impedance_rlc_series, impedance_rlc_parallel 등 계산 함수들)
# ... (이전 코드 리뷰에서 수정된 함수들을 여기에 붙여넣으세요) ...
...
def impedance_rlc_series(R: float, L: float, C: float, f: float) -> Dict[str, Any]:
    ...
    return {
        "Z_total [Ω]": f"{Z_complex.real:.4f} + {Z_complex.imag:.4f}j",
        "Magnitude |Z| [Ω]": abs(Z_complex),
        "Phase [°]": math.degrees(cmath.phase(Z_complex)),
    }
...
def impedance_rlc_parallel(R: float | None, L: float | None, C: float | None, f: float) -> Dict[str, Any]:
    ...
    if Y_total == 0: return {"error": "모든 소자가 없거나 공진 상태일 수 있습니다."}
    Z_total = 1 / Y_total
    return {
        "Z_total [Ω]": f"{Z_total.real:.4f} + {Z_total.imag:.4f}j",
        "Magnitude |Z| [Ω]": abs(Z_total),
        "Phase [°]": math.degrees(cmath.phase(Z_total)),
    }

# --- API 엔드포인트 정의 ---
@app.get("/")
def read_root():
    return {"message": "전기·전자 종합 어시스턴트 API 서버"}
...
async def handle_chat(request: ChatRequest):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a helpful assistant specializing in electrical engineering."),
...
async def upload_document(file: UploadFile = File(...)):
    """PDF 문서를 업로드하고 처리하여 세션을 생성하는 엔드포인트"""
